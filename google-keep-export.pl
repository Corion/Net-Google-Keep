...
# This would be for really manual scraping the generated HTML from the application
# not fun

#my @notes = $m->xpath('//div[@tabindex][@style][div/div[div[@aria-label = "Title"]]]');
my @notes = $m->xpath('//div[div/div/div[@aria-label = "Pin note"]]');

# $note->xpath(//@aria-label = "Note") => note body
# $note->xpath(//@aria-label = "Title") => title
# $note->xpath(//@aria-role  = "checkbox") => list?
# $note->xpath(//label) => labels

use HTML::TreeBuilder::XPath;

for my $note (@notes) {
    if( (my $body = $note->get_attribute('outerHTML')) =~ /\bautomation\b/ ) {
        my $t = HTML::TreeBuilder::XPath->new_from_content($body);
    
        # All content-carrying elements:
        for my $node ($t->findnodes('//*[contains(@class,"notranslate")]')) {
            print $node->as_XML_compact, "\n";
        };
        # Also find the change timestamp
        #print $_->as_XML_compact,"\n" for $t->findnodes('//*[contains(text(),"Edited")]');
        print $_->as_XML_compact,"\n" for $t->findnodes('//*[contains(@aria-label,"Created ")]');
        print "\n";
    
        my $title_class = $t->findvalue('//*[@aria-label = "Title"]/@class');
        warn "Title: $title_class";
        
        # Determine CSS classname for note id
        # Determine CSS classname for Note
        # Determine CSS classname for Title
        # Determine CSS classname for checkbox
        # Determine CSS classname for labels
        my @labels = $t->findnodes('//label');
        warn $_->findvalue('//label/text()') for @labels;

    #warn $note->get_attribute('class');
    #warn $note->get_attribute('style');
    #warn $body;
    };
};

#my $r = $m->httpResponseFromChromeResponse( $notes );
#warn $r->decoded_content;

# Extract the token from Chrome
#my( $value, $type ) = $m->eval('_docs_flag_initialData');

# notes#key ?
#my $token = $value->{n_k};

# https://clients6.google.com/notes/v1/changes?alt=json&key=:token
